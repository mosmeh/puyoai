...
const int FEATURES = 2 * FEATURES_FOR_ONE_PLAYER + 3 + 22 * PLAN_FEATURES;

class FeatureExtractor {
public:
    static std::valarray<float> extract(const State& state) {
        std::valarray<float> features(0.0, FEATURES);
        ...
        for (int i = 0; i < NUM_DECISIONS; ++i) {
            if (plans[i].first) {
                ...
                features[index++] = plan.rensaResult().chains / 19.0;
                features[index++] = static_cast<float>(plan.hasZenkeshi());
                features[index++] = 1.0 / (plan.numChigiri() + 1);
                features[index++] = 1.0 / (plan.framesToIgnite() + 1);
                features[index++] = 1.0 / (plan.totalFrames() + 1);
                features[index++] = static_cast<float>(plan.field().countReachableSpaces()) / FIELD_SIZE;
                ...
                features[index++] = max_chains / 19.0;
...
private:
    static constexpr float FIELD_HEIGHT = static_cast<float>(FieldConstant::HEIGHT + 1);
    static constexpr float ASSUMED_MAX_FRAMES = 4000;
    static constexpr float ASSUMED_MAX_NUM_OJAMA = 4000;

    static std::valarray<float> extractForOnePlayer(const PlayerState& pstate, const int frameId) {
        std::valarray<float> features(0.0, FEATURES);
        ...
        features[index++] = static_cast<float>(pstate.field.countReachableSpaces()) / FIELD_SIZE;
        features[index++] = static_cast<float>(pstate.field.countUnreachableSpaces()) / FIELD_SIZE;
        features[index++] = static_cast<float>(pstate.isRensaOngoing());
        features[index++] = static_cast<float>(pstate.hasZenkeshi);
